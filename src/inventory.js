...
function itemVisual(item) {
    if (item.texture) {
        ...
        if (!slabClass && !stairClass && Number(item.id) !== 16) {
            ...
            const slab3dClass = slabClass ? " catalogSlab3d" : "";
            return `<span class="catalogIcon catalogBlock3d${slab3dClass}" style="${style}"><i class="blockFace blockFront"></i><i class="blockFace blockBack"></i><i class="blockFace blockRight"></i><i class="blockFace blockLeft"></i><i class="blockFace blockTop"></i><i class="blockFace blockBottom"></i></span><span class="catalogFallback">${item.name.charAt(0)}</span>`;
        }
        if (slabClass && Number(item.id) !== 16) {
            const faces = blockFaceTextures(item);
            const style = `--block-front:url('${faces.front}');--block-back:url('${faces.back}');--block-right:url('${faces.right}');--block-left:url('${faces.left}');--block-top:url('${faces.top}');--block-bottom:url('${faces.bottom}')`;
...
function createInventoryUI() {
    ...
    style.textContent = `
.stair3dIcon{position:absolute;left:50%;top:50%;width:40px;height:40px;transform:translate(-50%,-50%) rotateX(-30deg) rotateY(45deg) scale(.72);transform-style:preserve-3d;transform-origin:center center;pointer-events:none;background:transparent;overflow:visible;z-index:2}
...
.stair3dIcon .stairStepLower .stairBottom{transform:rotateX(-90deg) translateZ(10px);filter:brightness(.62)}
...
.stair3dIcon .stairStepUpper .stairRight{left:10px;transform:rotateY(90deg) translateZ(20px);filter:brightness(.78)}
.stair3dIcon .stairStepUpper .stairLeft{left:10px;transform:rotateY(-90deg) translateZ(20px);filter:brightness(.88)}
...
#catalogSearchWrap{width:min(300px,45%);height:30px;display:flex;align-items:center;border:2px solid #121212;background:#2a2a2a;box-shadow:inset 2px 2px 0 #181818}
#catalogSearchWrap .searchIcon{font-size:19px;color:#aaa;padding:0 5px 2px}
