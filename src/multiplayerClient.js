...
window.__webminecraftMultiplayerRoomInfo = { room: "", serverName: "", private: false };
...
function ensureMenu() {
    ...
    roomCreateButton.addEventListener("click", () => { const open = roomView.classList.toggle("create-open"); roomCreateButton.textContent = open ? "× Cancel" : "+ Create Room"; if (open) { roomInput.value = ""; setServerType(false); setKeepOpen24h(false); joinButton.disabled = false; requestAnimationFrame(() => roomInput.focus()); } else { roomInput.value = ""; setKeepOpen24h(false); } });
    ...
    createAndJoinButton.addEventListener("click", () => { const roomName = (roomInput.value.trim() || "").slice(0,32); if (!roomName) { roomInput.focus(); setStatus("Enter a room name first.", true); return; } if (roomName.toLowerCase() === "player") { roomInput.focus(); setStatus("The room name \"player\" is reserved. Choose another room name.", true); return; } joinButton.disabled = false; joinButton.click(); });
