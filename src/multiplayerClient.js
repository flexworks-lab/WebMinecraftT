...
function makeStyle() {
    ...
    style.textContent = `
        ...
        #multiplayerServerDetails{position:fixed;right:24px;top:94px;width:min(430px,calc(100% - 48px));max-height:calc(100% - 118px);overflow:auto;display:none;background:#252525;border:2px solid #101010;border-top-color:#777;border-left-color:#777;box-shadow:8px 8px 0 rgba(0,0,0,.55);z-index:5;color:#fff}
        #multiplayerServerDetails.open{display:block}
        #multiplayerServerDetails .sw2-panel-head{padding:15px;border-bottom:2px solid #111;display:flex;align-items:center;gap:12px}
        #multiplayerServerDetails .sw2-panel-head h2{margin:0 auto 0 0;font-family:"MinecraftFont",monospace;font-size:20px;text-shadow:2px 2px 0 #000}
        #multiplayerServerDetails .sw2-panel-body{padding:18px}
        #multiplayerServerDetails .sw2-label{color:#aaa;font-size:12px;margin:0 0 7px}
        #multiplayerServerDetails .sw2-value{padding:12px;background:#111;border:1px solid #555;font-family:Arial,sans-serif;font-size:11px;line-height:1.5;word-break:break-word}
        #multiplayerServerDetails .sw2-status{display:inline-block;margin-bottom:14px;padding:5px 8px;background:#20261f;border:1px solid #465241;color:#a7d57f;font-size:10px;font-weight:800}
        #multiplayerServerDetails .sw2-status.offline{color:#df9387}
        #multiplayerServerDetails .sw2-help{color:#aaa;font-size:12px;line-height:1.5;margin:12px 0 15px}
        #multiplayerServerDetails .sw2-btn{width:100%;min-height:38px;padding:8px;background:linear-gradient(#686868,#505050);color:#fff;border:2px solid #111;border-top-color:#888;border-left-color:#888;cursor:pointer;font-family:"MinecraftFont",monospace;font-size:10px;text-shadow:2px 2px 0 #222;box-sizing:border-box;margin:5px 0}
        ...
        #multiplayerServerDetails .sw2-room{padding:9px 10px;margin:5px 0;background:#343434;border:1px solid #4b4b4b;color:#ddd;font-size:10px;line-height:1.5}
        @media(max-width:760px){#multiplayerServerDetails{right:12px;top:12px;width:calc(100% - 24px);max-height:calc(100% - 24px)}}
...
function ensureMenu() {
    ...
    const showRoomView = server => { selectedServer = server; overlay.querySelector("#multiplayerPanel")?.classList.remove("servers-screen"); overlay.querySelector("#multiplayerPanel")?.classList.add("rooms-screen"); roomView.classList.add("roomsStyle"); roomView.classList.remove("create-open"); serverView.style.display = "none"; roomView.style.display = "block"; stepServer.classList.remove("active"); stepRoom.classList.add("active"); serverInput.value = server.websocket || defaultServerUrl(); selectedInfo.innerHTML = `<strong>${escapeHtml(server.name || "Server")}</strong> · ${escapeHtml(server.description || "Multiplayer server")}`; renderRoomList(server); joinButton.disabled = false; setStatus(""); backButton.textContent = "Back to Servers"; };
    const openServerDetails = server => { if (!server || !serverDetails) return; selectedServer = server; const online = server.online !== false; const rooms = Array.isArray(server.rooms) ? server.rooms : []; serverDetails.innerHTML = `<div class="sw2-panel-head"><h2>${escapeHtml(server.name || "Server")}</h2><button class="sw2-btn" style="width:38px;margin:0;padding:0" type="button" data-server-details-close aria-label="Close">×</button></div><div class="sw2-panel-body"><span class="sw2-status${online ? "" : " offline"}">${online ? "● ONLINE" : "○ OFFLINE"}</span><p class="sw2-label">Description</p><div class="sw2-value">${escapeHtml(server.description || "Multiplayer server")}</div><p class="sw2-label" style="margin-top:12px">Players</p><div class="sw2-value">${Number(server.players) || 0}${Number(server.maxPlayers) ? "/" + Number(server.maxPlayers) : ""} online</div><p class="sw2-help">${rooms.length ? "This server has " + rooms.length + " listed room" + (rooms.length === 1 ? "" : "s") + ". Choose Join Server to select a room." : "No rooms are listed yet. You can still use the server connection to create or join a room."}</p>${rooms.slice(0,8).map(room => `<div class="sw2-room"><strong>${escapeHtml(room.name || room.id || "Room")}</strong> · ${Number(room.players) || 0}${Number(room.maxPlayers) ? "/" + Number(room.maxPlayers) : ""} online${room.private || room.isPrivate ? " · 🔒 Private" : ""}</div>`).join("")}<button class="sw2-btn sw2-green" type="button" data-server-join ${online ? "" : "disabled"}>Join Server</button><button class="sw2-btn" type="button" data-server-details-close>Back to Servers</button></div>`; serverDetails.classList.add("open"); serverDetails.setAttribute("aria-hidden","false"); serverDetails.querySelectorAll("[data-server-details-close]").forEach(button => button.addEventListener("click", closeServerDetails)); const join = serverDetails.querySelector("[data-server-join]"); join?.addEventListener("click", () => { closeServerDetails(); showRoomView(server); }); };
    const renderServers = servers => { serverData = servers; serverList.innerHTML = ""; if (!servers.length) { serverList.innerHTML = '<div class="multiplayerEmpty">No servers found.</div>'; return; } for (const server of servers) { const button = document.createElement("button"); button.type = "button"; button.className = "multiplayerCard"; const online = server.online !== false; button.innerHTML = `<div class="multiplayerCardPreview" aria-hidden="true"></div><div class="multiplayerCardBody"><div class="multiplayerCardTop"><span class="multiplayerCardName">${escapeHtml(server.name || "Server")}</span><span class="${online ? "multiplayerOnline" : "multiplayerOffline"}">${online ? "● ONLINE" : "○ OFFLINE"}</span></div><div class="multiplayerMeta">${escapeHtml(server.description || "Multiplayer server")} · ${online ? "Ready to join" : "Unavailable"}<br>${Number(server.players) || 0}${Number(server.maxPlayers) ? "/" + Number(server.maxPlayers) : ""} players online</div></div><span class="multiplayerCardAction">${online ? "View" : "Offline"}</span>`; button.addEventListener("click", () => openServerDetails(server)); serverList.appendChild(button); } };
...
export function sendPlayerState(position, rotation, heldItemId = 0, action = null) { if (!isMultiplayerActive()) return; const outgoingAction = pendingPlayerAction !== "idle" ? pendingPlayerAction : (action || "idle"); pendingPlayerAction = "idle"; socket.send(JSON.stringify({ type: "player_state", position: { x: Number(position?.x) || 0, y: Number(position?.y) || 0, z: Number(position?.z) || 0 }, rotation: { x: Number(rotation?.x) || 0, y: Number(rotation?.y) || 0, z: Number(rotation?.z) || 0 }, heldItemId: Math.max(0, Math.floor(Number(heldItemId) || 0)), action: ["idle", "walk", "mine", "place", "jump"].includes(outgoingAction) ? outgoingAction : "idle" })); }
