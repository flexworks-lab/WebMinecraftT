...
function ensureChatUI() {
    ...
    style.textContent = `
        ...
        .multiplayerVerifiedBadge{display:inline-flex;align-items:center;gap:3px;margin-left:0;margin-right:4px;padding:2px 5px;border-radius:999px;font:900 9px/1 Arial,sans-serif;white-space:nowrap;text-shadow:1px 1px 0 rgba(0,0,0,.35);vertical-align:1px}
        .multiplayerVerifiedDeveloper{background:#7d2020;border:1px solid #ff6b5f;color:#fff0ed}
        .multiplayerVerifiedMain{background:#6f5217;border:1px solid #d8ae48;color:#fff3bd}
        .multiplayerVerifiedAdmin{background:#245a96;border:1px solid #5ea9ff;color:#eaf5ff}
    ...
    window.__webminecraftChatAdd = (text, system = false, name = "", isAdmin = false, verifiedRole = "") => {
        ...
        if (system) line.textContent = text;
            ...
            if (verifiedRole === "developer" || verifiedRole === "main" || verifiedRole === "admin") {
                const verify = document.createElement("span");
                verify.className = `multiplayerVerifiedBadge multiplayerVerified${verifiedRole === "developer" ? "Developer" : verifiedRole === "main" ? "Main" : "Admin"}`;
                verify.textContent = verifiedRole === "developer" ? "✓ Developer" : verifiedRole === "main" ? "★ Main Admin" : "✓ Verified Admin";
                verify.title = verify.textContent;
                verify.setAttribute("aria-label", verify.textContent);
...
function ensureMenu() {
    ...
    const getGlobalVerifiedChatRole = async () => {
        const user = window.firebase?.auth?.().currentUser || null;
        if (!user) return "";
        if (String(user.email || "").trim().toLowerCase() === "worthmarcus19@gmail.com") return "developer";
        try {
            const snap = await window.firebase?.firestore?.().collection("admins").doc(user.uid).get();
