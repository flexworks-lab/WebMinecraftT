...
.adminManageStatus{min-height:18px;font-size:11px;color:#9fce72;margin:5px 0}.adminManageStatus.error{color:#e38a7b}.adminManageList{display:flex;flex-direction:column;gap:6px;max-height:220px;overflow:auto}.adminManageItem{display:flex;align-items:center;gap:8px;padding:8px;background:#151515;border:1px solid #444;font-size:11px}.adminManageUid{flex:1;word-break:break-all}.adminManageToggle{background:#633f3b;color:#fff;border:1px solid #111;padding:6px 9px;font-size:9px;cursor:pointer}.adminManageToggle.on{background:#526f3c}@media(max-width:650px){.adminManageRow{flex-direction:column}.adminManageRow .devButton{width:100%}}
...
async function refresh(){
 ...
 try{
  ...
  docs.forEach(doc=>{const data=doc.data()||{},uid=String(data.uid||doc.id),enabled=data.enabled===true,row=document.createElement("div");row.className="adminManageItem";row.innerHTML=`<span class="adminManageUid">${esc(uid)}</span><button class="adminManageToggle ${enabled?"on":""}" type="button">${enabled?"Turn Off":"Turn On"}</button>`;row.querySelector("button").onclick=async()=>{try{await firestore.collection(ADMIN_COLLECTION).doc(doc.id).set({uid,enabled:!enabled,updatedAt:new Date()},{merge:true});await refresh();}catch(e){alert(e?.message||"Could not change admin status.");}};list.appendChild(row);});
