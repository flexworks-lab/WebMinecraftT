...
#survivalMiningProgress.active{display:block}
#survivalMiningProgress .bar{height:4px;margin-top:4px;background:#222;border:1px solid #111;overflow:hidden}
#survivalMiningProgress .fill{height:100%;width:0;background:#fff}
`;
...
function updateCracks(overlay, progress) {
    ...
    if (hud) {
        hud.querySelector(".fill").style.width = `${Math.round(progress * 100)}%`;
        hud.querySelector(".label").textContent = `Breaking ${Math.round(progress * 100)}%`;
...
function destroyCracks(overlay) {
    ...
    for (const mesh of overlay.stages) {
...
function burst(center, type) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        p.userData.velocity = new THREE.Vector3((Math.random()-.5)*2.1,.9+Math.random()*1.8,(Math.random()-.5)*2.1);
    ...
    function tick(time) {
        ...
        for (const p of particles) {
            ...
            alive = true;
            p.userData.velocity.y -= .085;
            p.position.addScaledVector(p.userData.velocity, .016);
            p.rotation.x += .1; p.rotation.y += .08;
...
function createDrop(type, position) {
    ...
    if (textureName) {
        ...
        material = new THREE.MeshLambertMaterial({ color: COLORS[type] ?? 0xaaa });
    }
...
function mergeDrops() {
    for (let i = drops.length - 1; i >= 0; i--) {
        ...
        for (let j = i - 1; j >= 0; j--) {
            const b = drops[j];
            if (!b.parent || a.userData.type !== b.userData.type) continue;
            if (a.position.distanceTo(b.position) > .7) continue;
...
function updateDrops(time) {
    ...
    for (let i = drops.length - 1; i >= 0; i--) {
        ...
        if (time - drop.userData.spawnedAt >= DESPAWN_MS) {
            drop.parent.remove(drop); drops.splice(i,1); continue;
        }
        const baseY = drop.position.y;
        drop.userData.baseY ??= baseY;
        drop.position.y = drop.userData.baseY + Math.sin(time * .003 + drop.userData.bob) * .045;
        drop.rotation.y += .018;
        const distance = drop.position.distanceTo(player);
        if (distance <= PICKUP_RANGE) {
            const direction = player.clone().sub(drop.position);
            const strength = Math.min(.2, Math.max(.035, (PICKUP_RANGE - distance) * .12));
            drop.position.addScaledVector(direction.normalize(), strength);
        }
        if (drop.position.distanceTo(player) < .5) {
            if (addToInventory(drop.userData.type, drop.userData.count)) {
                drop.parent.remove(drop); drops.splice(i,1);
            }
        }
    }
}
...
function cancelMining() {
    ...
    mining = null;
    document.getElementById("survivalMiningProgress")?.classList.remove("active");
}
