...
async function loadAdminServers() {
    ...
    try {
        ...
        servers.forEach(server => {
            ...
            chat.slice(-100).forEach(message => { const line = document.createElement("div"); line.className = "adminChatLine"; line.innerHTML = `<strong>${esc(message.name || "Player")}:</strong> ${esc(message.text || "")}`; feed.appendChild(line); });
