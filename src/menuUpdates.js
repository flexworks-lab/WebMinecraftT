...
function setupPauseMenu(){
    ...
    style.textContent=[
        "#pauseMenu{position:fixed;inset:0;display:none;background:rgba(0,0,0,.66);color:#fff;z-index:2000000!important;pointer-events:auto;font-family:Arial,sans-serif}",
        "#pauseMenu.pauseOpen{display:block}",
        "#pauseShell{position:absolute;inset:0;display:grid;grid-template-columns:minmax(460px,1fr) minmax(300px,34vw);background:linear-gradient(180deg,rgba(25,29,26,.94),rgba(14,17,15,.97))}",
        "#pauseMain{position:relative;min-width:0;display:flex;flex-direction:column;align-items:center;padding:clamp(34px,7vh,72px) 7vw 28px;box-sizing:border-box}",
        "#pauseLogo{margin:0 0 clamp(30px,7vh,64px);font-family:Arial Black,Arial,sans-serif;font-size:clamp(42px,6vw,76px);font-weight:900;letter-spacing:1px;color:#fff;text-shadow:4px 4px 0 #171717,7px 7px 0 rgba(0,0,0,.38);text-align:center;transform:skewY(-2deg)}",
        "#pauseLogoSub{margin-top:-32px;margin-bottom:26px;color:#a9cf87;font:800 11px Arial,sans-serif;letter-spacing:4px;text-shadow:0 2px 0 #111}",
        "#pauseNav{width:min(390px,100%);margin-left:-7vw;display:grid;gap:11px}",
        ".pauseButton{width:100%;min-height:52px;padding:11px 16px;border:2px solid #111;border-top-color:#919791;border-left-color:#919791;background:linear-gradient(#6b6d6b,#505451);color:#fff;font-family:Arial Black,Arial,sans-serif;font-size:13px;text-align:left;cursor:pointer;text-shadow:2px 2px 0 #222;box-shadow:inset 2px 2px 0 rgba(255,255,255,.08),inset -3px -3px 0 rgba(0,0,0,.28),0 4px 0 #111}",
        ...
        "#pauseNavSpacer{height:clamp(16px,3vh,28px)}#pauseQuit{margin-top:auto;background:linear-gradient(#5c5c5c,#434343)}",
    ...
    const makeInvite=async()=>{const info=getRoomInfo();const text=info.active?"Join me in WebMinecraftT! Server: "+(info.server||"WebMinecraft server")+". Room: "+(info.room||"default")+" .":"Join me in WebMinecraftT! I am playing a world right now.";try{if(navigator.share)await navigator.share({title:"WebMinecraftT Invite",text});else if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(text);overlay.querySelector("#pauseInviteStatus").textContent=navigator.share?"Invite opened.":"Invite copied to clipboard.";}catch{overlay.querySelector("#pauseInviteStatus").textContent="Invite cancelled.";}window.setTimeout(()=>{const el=overlay.querySelector("#pauseInviteStatus");if(el)el.textContent="";},2200);};
    ...
    const close=event=>{event?.preventDefault();event?.stopPropagation();paused=false;overlay.classList.remove("pauseOpen");overlay.setAttribute("aria-hidden","true");if(isGameRunning())document.body.requestPointerLock?.();};
    ...
    overlay.querySelector("#pauseQuit").addEventListener("click",()=>{const saveAndReturn=async()=>{try{window.webminecraftSaveWorldPreview?.(getWorldSeed());await window.webminecraftSaveCurrentWorld?.();}catch{}window.location.reload();};void saveAndReturn();});
