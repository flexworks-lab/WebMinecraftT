...
function isFlatWorld() {
    try { return localStorage.getItem(`${WORLD_TYPE_PREFIX}${WORLD_SEED}`) === "flat"; }
    catch { return false; }
}
...
const chunkMaterials = [
    ...
    cobblestoneMaterial, gravelMaterial,
    sandstoneMaterial[0], sandstoneMaterial[2], sandstoneMaterial[3],
...
function valueNoise3D(x, y, z, scale = 1, salt = 0) { const sx=x/scale,sy=y/scale,sz=z/scale,x0=Math.floor(sx),y0=Math.floor(sy),z0=Math.floor(sz),tx=fade(sx-x0),ty=fade(sy-y0),tz=fade(sz-z0),n000=hash3D(x0,y0,z0,salt),n100=hash3D(x0+1,y0,z0,salt),n010=hash3D(x0,y0+1,z0,salt),n110=hash3D(x0+1,y0+1,z0,salt),n001=hash3D(x0,y0,z0+1,salt),n101=hash3D(x0+1,y0,z0+1,salt),n011=hash3D(x0,y0+1,z0+1,salt),n111=hash3D(x0+1,y0+1,z0+1,salt),nx00=lerp(n000,n100,tx),nx10=lerp(n010,n110,tx),nx01=lerp(n001,n101,tx),nx11=lerp(n011,n111,tx),nxy0=lerp(nx00,nx10,ty),nxy1=lerp(nx01,nx11,ty); return lerp(nxy0,nxy1,tz); }
...
export function getTerrainProfile(x,z){if(isFlatWorld())return{height:FLAT_SURFACE_Y,continentalness:1,erosion:0,peaks:0,detail:0};const continentalness=octave2D(x,z,4,320,.52,61),erosion=octave2D(x+1400,z-800,3,160,.54,73),peaks=octave2D(x-600,z+1100,4,120,.5,89),detail=octave2D(x+2400,z-1700,3,28,.5,97);let baseHeight=21+(continentalness-.5)*21;baseHeight+=(.5-erosion)*10;const mountainMask=Math.max(0,(peaks-.57)/.43);baseHeight+=mountainMask*mountainMask*30;baseHeight+=(detail-.5)*5;const oceanMask=Math.max(0,.20-continentalness)/.20;baseHeight-=oceanMask*7;return{height:Math.floor(THREE.MathUtils.clamp(baseHeight,MIN_Y+4,WORLD_TOP-8)),continentalness,erosion,peaks,detail};}
...
function getUnderwaterBlock(x,y,z,surfaceY){const depth=surfaceY-y,surfaceRoll=hash3D(x,y,z,1701),blockRoll=hash3D(x,y,z,1707);if(depth<=0){if(surfaceRoll<.50)return BLOCK.DIRT;if(surfaceRoll<.79)return BLOCK.SAND;if(surfaceRoll<.94)return BLOCK.GRAVEL;return BLOCK.STONE;}if(depth<=4){if(blockRoll<.50)return BLOCK.DIRT;if(blockRoll<.72)return BLOCK.SAND;if(blockRoll<.91)return BLOCK.GRAVEL;return BLOCK.STONE;}if(blockRoll<.32)return BLOCK.DIRT;if(blockRoll<.52)return BLOCK.SAND;if(blockRoll<.82)return BLOCK.GRAVEL;return chooseStoneVariant(x,y,z,surfaceY);}
function getSurfaceBlock(biome,y,surfaceY,x,z){if(isFlatWorld()){if(y===surfaceY)return BLOCK.GRASS;if(y>=surfaceY-3)return BLOCK.DIRT;return BLOCK.STONE;}const submerged=surfaceY<SEA_LEVEL,beach=!submerged&&surfaceY<=SEA_LEVEL+1;if(submerged)return getUnderwaterBlock(x,y,z,surfaceY);if(biome==="desert"){if(y>=surfaceY-4)return BLOCK.SAND;if(y>=surfaceY-7)return BLOCK.SANDSTONE;return chooseStoneVariant(x,y,z,surfaceY);}if(biome==="badlands"){if(y===surfaceY)return BLOCK.SAND;if(y>=surfaceY-5)return BLOCK.SANDSTONE;return chooseStoneVariant(x,y,z,surfaceY);}if(biome==="snow"||biome==="tundra"){if(y===surfaceY)return BLOCK.SNOW;if(y>=surfaceY-4)return BLOCK.DIRT;return chooseStoneVariant(x,y,z,surfaceY);}if(beach){if(y>=surfaceY-1)return BLOCK.SAND;if(y===surfaceY-2)return BLOCK.SANDSTONE;}if(y===surfaceY)return BLOCK.GRASS;if(y>=surfaceY-3)return BLOCK.DIRT;return chooseStoneVariant(x,y,z,surfaceY);}
...
export function isPointInWater(x,y,z){if(isFlatWorld())return false;const profile=getTerrainProfile(Math.floor(x),Math.floor(z));if(profile.height>=SEA_LEVEL)return false;const waterSurface=SEA_LEVEL+.42,solidFloor=profile.height+.5;return y<waterSurface-.02&&y>solidFloor+.05;}
function treeChance(x,z){if(isFlatWorld())return 0;const{temperature,humidity}=getClimate(x,z);if(temperature<.28||humidity<.3)return 0;const forest=THREE.MathUtils.clamp((humidity-.34)/.34,0,1),base=hash2D(x,z,1201),jitter=hash2D(x+137,z-411,1207),density=humidity>.6?.18+forest*.14:.045+forest*.045;return(base*.78+jitter*.22)<density?1:0;}
...
function getChunkNeighborType(chunk,lx,y,lz,dx,dz){
    ...
    if(nx>=0&&nx<CHUNK_SIZE&&nz>=0&&nz<CHUNK_SIZE&&y>=MIN_Y&&y<=WORLD_TOP){
        return chunk.blocks[blockIndex(nx,y,nz)]||BLOCK.AIR;
    }
    return getBlockType(chunk.x*CHUNK_SIZE+nx,y,chunk.z*CHUNK_SIZE+nz);
}
function makeGeometryForChunk(chunk){
    ...
    for(let lx=0;lx<CHUNK_SIZE;lx++)for(let lz=0;lz<CHUNK_SIZE;lz++){
        ...
        for(let y=MIN_Y;y<=WORLD_TOP;y++){
            ...
            for(let faceIndex=0;faceIndex<6;faceIndex++){
                const face=FACES[faceIndex],neighborY=y+face.normal[1],neighbor=getChunkNeighborType(chunk,lx,neighborY,lz,face.normal[0],face.normal[2]);
...
function disposeChunkMesh(chunk){if(!chunk||!worldScene)return;const key=chunkKey(chunk.x,chunk.z),mesh=chunkMeshes.get(key);if(mesh){worldScene.remove(mesh);mesh.geometry.dispose();chunkMeshes.delete(key);}}
