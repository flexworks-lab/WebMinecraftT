...
function getUnderwaterShade(surfaceY,y,x){if(surfaceY>=SEA_LEVEL||y>surfaceY)return 1;const depth=Math.max(0,SEA_LEVEL-(y+.5)),depthT=THREE.MathUtils.clamp(depth/24,0,1),shade=THREE.MathUtils.lerp(1,.43,depthT),variation=.96+hash3D(x,y,0,1911)*.06;return THREE.MathUtils.clamp(shade*variation,.40,1);}
function makeGeometryForChunk(chunk){const positions=[],normals=[],uvs=[],colors=[],groups=Array.from({length:chunkMaterials.length},()=>[]);let vertexCount=0;for(let lx=0;lx<CHUNK_SIZE;lx++){for(let lz=0;lz<CHUNK_SIZE;lz++){const x=chunk.x*CHUNK_SIZE+lx,z=chunk.z*CHUNK_SIZE+lz,surfaceY=getTerrainProfile(x,z).height;for(let y=MIN_Y;y<=WORLD_TOP;y++){const type=chunk.blocks[blockIndex(lx,y,lz)];if(!isSolid(type))continue;const underwaterShade=getUnderwaterShade(surfaceY,y,x);for(let faceIndex=0;faceIndex<6;faceIndex++){const face=FACES[faceIndex],neighbor=getBlockType(x+face.normal[0],y+face.normal[1],z+face.normal[2]);if(isSolid(neighbor)&&neighbor!==BLOCK.LEAVES)continue;const base=vertexCount;for(const corner of face.corners){positions.push(x+corner[0],y+corner[1],z+corner[2]);normals.push(face.normal[0],face.normal[1],face.normal[2]);colors.push(underwaterShade,underwaterShade,underwaterShade);}uvs.push(0,0,0,1,1,1,1,0);const matIndex=materialIndexFor(type,faceIndex);groups[matIndex].push(base,base+1,base+2,base+3,base,base+2);vertexCount+=4;}}}}if(vertexCount===0)return null;const geometry=new THREE.BufferGeometry();geometry.setAttribute("position",new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute("normal",new THREE.Float32BufferAttribute(normals,3));geometry.setAttribute("uv",new THREE.Float32BufferAttribute(uvs,2));geometry.setAttribute("color",new THREE.Float32BufferAttribute(colors,3));const index=[];for(let i=0;i<groups.length;i++){const start=index.length;index.push(...groups[i]);if(groups[i].length)geometry.addGroup(start,groups[i].length,i);}geometry.setIndex(index);geometry.computeBoundingSphere();geometry.computeBoundingBox();return geometry;}
...
export function updateChunkVisibility(position,camera){if(!worldScene||!position)return;const{chunkX,chunkZ}=getChunkCoords(position.x,position.z);if(chunkX!==lastPlayerChunkX||chunkZ!==lastPlayerChunkZ){lastPlayerChunkX=chunkX;lastPlayerChunkZ=chunkZ;queueNeededChunks(chunkX,chunkChunkZ);unloadFarChunks(chunkX,chunkZ);}processChunkQueue();updateFacingVisibility(position,camera);}
