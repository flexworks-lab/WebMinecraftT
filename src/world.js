...
function updateFacingVisibility(playerPosition,camera){if(!camera)return;const direction=new THREE.Vector3();camera.getWorldDirection(direction);direction.y=0;if(direction.lengthSq()<.0001)return;direction.normalize();const playerChunk=getChunkCoords(playerPosition.x,playerPosition.z);const minVisibleDot=Math.cos(THREE.MathUtils.degToRad(105));for(const chunk of chunks.values()){const mesh=chunkMeshes.get(chunkKey(chunk.x,chunk.z)),water=chunk.waterMesh;if(!mesh&&!water)continue;const cx=chunk.x*CHUNK_SIZE+CHUNK_SIZE*.5,cz=chunk.z*CHUNK_SIZE+CHUNK_SIZE*.5,vx=cx-playerPosition.x,vz=cz-playerPosition.z,distance=Math.hypot(vx,vz),dot=distance>0?(vx*direction.x+vz*direction.z)/distance:1,visible=Math.max(Math.abs(chunk.x-playerChunk.chunkX),Math.abs(chunk.z-playerChunk.chunkZ))<=1||dot>minVisibleDot;if(mesh)mesh.visible=visible;if(water)water.visible=visible;}}
export function setBlockAt(x,y,z,type){x=Math.round(x);y=Math.round(y);z=Math.round(z);const {chunkX,chunkZ,localX,localZ}=getChunkCoords(x,z),chunk=getChunk(chunkX,chunkZ);if(!chunk||y<MIN_Y||y>=MIN_Y+CHUNK_HEIGHT)return false;chunk.blocks[blockIndex(localX,y,localZ)]=type;rebuildChunkMesh(chunk);if(localX===0)rebuildChunkMesh(getChunk(chunkX-1,chunkZ));if(localX===CHUNK_SIZE-1)rebuildChunkMesh(getChunk(chunkX+1,chunkZ));if(localZ===0)rebuildChunkMesh(getChunk(chunkX,chunkZ-1));if(localZ===CHUNK_SIZE-1)rebuildChunkMesh(getChunk(chunkX,chunkZ+1));return true;}
export function updateVisibleChunks(playerX,playerZ){if(!worldScene)return;const {chunkX,chunkZ}=getChunkCoords(playerX,playerZ);queueNeededChunks(chunkX,chunkZ);unloadFarChunks(chunkX,chunkZ);}
export function updateChunkVisibility(position,camera){if(!worldScene)return;const {chunkX,chunkZ}=getChunkCoords(position.x,position.z);if(chunkX!==lastPlayerChunkX||chunkZ!==lastPlayerChunkZ){lastPlayerChunkX=chunkX;lastPlayerChunkZ=chunkZ;updateVisibleChunks(position.x,position.z);}processChunkQueue();updateFacingVisibility(position,camera);}
export function createWorld(scene){worldScene=scene;lastPlayerChunkX=Infinity;lastPlayerChunkZ=Infinity;generationQueue.length=0;queuedKeys.clear();chunks.clear();chunkMeshes.clear();updateVisibleChunks(0,0);}
