...
function addBlock(blocks, x, z) { blocks.push(new THREE.Vector3(x * CLOUD_BLOCK_SIZE, 0, z * CLOUD_BLOCK_SIZE)); }
function buildCloudShape(cellX, cellZ) {
    ...
    for (const block of blocks) { const key = `${block.x}|${block.z}`; if (!seen.has(key)) unique.set(key, block); }
...
function makeCloud(cellX, cellZ) {
    ...
    for (let i = 0; i < blocks.length; i++) { matrix.makeTranslation(blocks[i].x, blocks[i].y, blocks[i].z); mesh.setMatrixAt(i, matrix); }
    ...
    mesh.renderOrder = 10;
    const randomX = seedHash(cellX, cellZ, 500), randomZ = seedHash(cellX, cellZ, 510);
...
function createUndergroundLighting(scene) {
    ...
    if (!legacyDepthLightNeutralized) {
        for (const object of scene.children) {
            if (!object.isPointLight || object.color?.getHex?.() !== 0x9db6d2) continue;
            object.intensity = 0;
            try { Object.defineProperty(object, "intensity", { configurable: true, get(){return 0;}, set(){} }); } catch { object.intensity = 0; }
        }
...
function removeWaterSpecularHighlights(scene) {
    scene.traverse(object => {
        ...
        if (material.specular?.set) material.specular.set(0x000000); else material.specular = 0x000000;
