...
function addChat(room, name, text, isAdmin = false) { const cleanText = String(text || "").replace(/[\r\n]+/g, " ").replace(/[<>]/g, "").trim().slice(0, MAX_CHAT_LENGTH); if (!cleanText) return false; room.adminChat ||= []; room.adminChat.push({ name, text: cleanText, system: false, isAdmin: Boolean(isAdmin), time: Date.now() }); while (room.adminChat.length > 100) room.adminChat.shift(); return cleanText; }
