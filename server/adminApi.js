...
async function getFirebaseCerts() {
    ...
    const certs = await response.json(); const cacheControl = response.headers.get("cache-control") || ""; const match = cacheControl.match(/max-age=(\d+)/i); const maxAge = Math.max(300, Math.min(Number(match?.[1] || 3600), 86400));
    certCache = certs; certCacheExpiresAt = Date.now() + maxAge * 1000; return certs;
}
async function verifyFirebaseIdToken(token) {
    const parts = String(token).split("."); if (parts.length !== 3) return null;
    const [encodedHeader, encodedPayload, encodedSignature] = parts; let header, payload;
    try { header = JSON.parse(decodeBase64Url(encodedHeader)); payload = JSON.parse(decodeBase64Url(encodedPayload)); } catch { return null; }
    ...
    const now = Math.floor(Date.now() / 1000); if (!Number.isFinite(payload.exp) || payload.exp <= now) return null; if (Number.isFinite(payload.iat) && payload.iat > now + 300) return null;
    const certs = await getFirebaseCerts(); const certificate = certs?.[header.kid]; if (!certificate) return null;
    const verifier = crypto.createVerify("RSA-SHA256"); verifier.update(`${encodedHeader}.${encodedPayload}`); verifier.end();
    const signature = Buffer.from(encodedSignature.replace(/-/g, "+").replace(/_/g, "/"), "base64"); if (!verifier.verify(certificate, signature)) return null;
    return payload;
}
async function verifyToken(request) {
    const auth = String(request.headers.authorization || ""); const token = auth.startsWith("Bearer ") ? auth.slice(7).trim() : ""; if (!token) return null;
    try { return await verifyFirebaseIdToken(token); } catch (error) { console.error("Firebase authentication failed:", error?.message || error); return null; }
}
...
function addChat(room, name, text) { const cleanText = String(text || "").replace(/[\r\n]+/g, " ").replace(/[<>]/g, "").trim().slice(0, MAX_CHAT_LENGTH); if (!cleanText) return false; room.adminChat ||= []; room.adminChat.push({ name, text: cleanText, system: false, time: Date.now() }); while (room.adminChat.length > 100) room.adminChat.shift(); return cleanText; }
function broadcast(room, message) { const payload = JSON.stringify(message); for (const player of room.players.values()) if (player.ws?.connected) player.ws.sendText(payload); }
async function readBody(request) { return await new Promise(resolve => { let body = ""; request.on("data", chunk => { body += chunk; if (body.length > 64 * 1024) request.destroy(); }); request.on("end", () => { try { resolve(JSON.parse(body || "{}")); } catch { resolve(null); } }); request.on("error", () => resolve(null)); }); }

export async function handleAdminRequest(request, response, rooms, cleanRoom) {
    if (request.method === "OPTIONS") { response.writeHead(204, { "access-control-allow-origin": "*", "access-control-allow-headers": "Authorization, Content-Type", "access-control-allow-methods": "GET, POST, OPTIONS" }); response.end(); return true; }
    const auth = await authenticate(request);
    if (!auth) { sendJson(response, 403, { ok: false, error: "Admin access denied." }); return true; }
    if (request.method === "GET" && request.url === "/admin/servers") { sendJson(response, 200, { ok: true, role: auth.role, servers: [...rooms.values()].map(roomInfo), updatedAt: Date.now() }); return true; }
    if (request.method !== "POST" || request.url !== "/admin/action") { sendJson(response, 404, { ok: false, error: "Not found." }); return true; }
    const body = await readBody(request); const action = String(body?.action || ""); const room = rooms.get(String(body?.serverId || ""));
    if (action === "kick" || action === "warn" || action === "chat") {
        ...
        const text = addChat(room, auth.role === "developer" ? "Developer" : "Admin", body?.text); if (!text) return sendJson(response, 400, { ok: false, error: "Message is empty." }); broadcast(room, { type: "chat_message", playerId: auth.role, name: auth.role === "developer" ? "Developer" : "Admin", text }); sendJson(response, 200, { ok: true }); return true;
    }
