...
function updateHeldDoor() {
    ...
    const isDoor = Number(selectedSlot?.dataset.itemId || selectedSlot?.getAttribute("data-item-id") || 0) === DOOR_BLOCK || !!selectedSlot?.querySelector('.hotbarTexture[style*="oak_door_bottom"]');
    if (!isDoor) {
...
function createDoorMesh(x, y, z, widthAxis = "x", openAngle = 0) {
    if (!scene) return null;
    const group = new THREE.Group();
    if (widthAxis === "x") group.position.set(x - 0.43, y, z);
    else {
        group.position.set(x, y, z - 0.43);
    ...
    group.userData.widthAxis = widthAxis;

    const panel = new THREE.Group();
    panel.rotation.y = openAngle;

    const bottom = new THREE.Mesh(new THREE.BoxGeometry(0.86, 1, 0.10), new THREE.MeshPhongMaterial({ map: DOOR_TEXTURE, color: 0xffffff, side: THREE.DoubleSide }));
    const top = new THREE.Mesh(new THREE.BoxGeometry(0.86, 1, 0.10), new THREE.MeshPhongMaterial({ map: DOOR_TOP_TEXTURE, color: 0xffffff, side: THREE.DoubleSide }));
...
function inferDoorWidthAxis(x, y, z) {
    const hasDoorX = getBlockAt(x - 1, y, z) === DOOR_BLOCK || getBlockAt(x + 1, y, z) === DOOR_BLOCK;
    const hasDoorZ = getBlockAt(x, y, z - 1) === DOOR_BLOCK || getBlockAt(x, y, z + 1) === DOOR_BLOCK;
    if (hasDoorZ && !hasDoorX) return "x";
    if (hasDoorX && !hasDoorZ) return "z";
    return "x";
}
...
function reconcileDoors(force = false) {
    ...
    for (let x = cx - DOOR_SCAN_RADIUS; x <= cx + DOOR_SCAN_RADIUS; x++) {
        for (let z = cz - DOOR_SCAN_RADIUS; z <= cz + DOOR_SCAN_RADIUS; z++) {
            for (let y = cy - 3; y <= cy + 4; y++) {
                if (getBlockAt(x, y, z) !== DOOR_BLOCK) continue;
                if (getBlockAt(x, y + 1, z) !== DOOR_BLOCK) continue;
                if (getBlockAt(x, y - 1, z) === DOOR_BLOCK) continue;
                const k = key(x, y, z);
                wanted.add(k);
                if (!doors.has(k)) createDoorMesh(x, y, z, inferDoorWidthAxis(x, y, z));
            }
        }
    }
    ...
    for (const [k, door] of doors) {
        ...
        const far = Math.max(Math.abs(x - cx), Math.abs(z - cz));
        const stillStored = getBlockAt(x, y, z) === DOOR_BLOCK && getBlockAt(x, y + 1, z) === DOOR_BLOCK;
        if (far > DOOR_SCAN_RADIUS + 3 || (!stillStored && !door.userData.open)) removeDoorMesh(x, y, z);
    }
}
...
function setDoorStoredState(door, open) {
    const { x, y, z } = door.userData.doorBase;
    if (open) {
...
function toggleDoor(door) {
    ...
    if (!setDoorStoredState(door, nextOpen)) return false;
    door.userData.open = nextOpen;
    const { x, z } = door.userData.doorBase;
    const side = door.userData.widthAxis === "x" ? camera.position.x - x : camera.position.z - z;
    door.userData.openAngle = nextOpen ? (side >= 0 ? Math.PI / 2 : -Math.PI / 2) : 0;
...
export function placeDoor(target) {
    ...
    if (getBlockAt(x, y - 1, z) === BLOCK.AIR) return false;
    const widthAxis = Math.abs(normal.z) >= Math.abs(normal.x) ? "x" : "z";
    if (!setBlockAt(x, y, z, DOOR_BLOCK)) return false;
    if (!setBlockAt(x, y + 1, z, DOOR_BLOCK)) {
        setBlockAt(x, y, z, BLOCK.AIR);
        return false;
    }
    createDoorMesh(x, y, z, widthAxis);
    selected = false;
...
if (document.body) {
    inventoryIconObserver.observe(document.body, { childList: true, subtree: true });
} else {
    window.addEventListener("DOMContentLoaded", () => inventoryIconObserver.observe(document.body, { childList: true, subtree: true }), { once: true });
}
